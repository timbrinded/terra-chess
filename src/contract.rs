...
use cosmwasm_std::{to_binary,Binary,Order, Deps, DepsMut, Env, MessageInfo, Response,StdError, StdResult};

use crate::msg::{InstantiateMsg, QueryMsg, ExecuteMsg, GameResult, GameMove, GameReponse, GameList, ChessMatch};
...
pub fn execute(
    ...
    let api = deps.api;
    match msg {
        ExecuteMsg::StartGame {opponent, start_move} => try_start_game(deps,info, opponent, start_move),
...
pub fn try_respond_game(deps: DepsMut, info: MessageInfo, host:String,  opp_move: GameMove) -> Result<Response, ContractError> {
    ...
    if game.opponent != opponent { return Err(ContractError::Unexplained {})};
...
pub fn resolve_game(host_move: GameMove, opp_move: GameMove) -> GameResult {
    ...
    if host_move == opp_move {return GameResult::Tie};
    if opp_move == GameMove::NotPlayed {return GameResult::Tie};

    match host_move{
        GameMove::Scissors => if opp_move == GameMove::Rock {
            GameResult::Loss
        } else { GameResult::Win},
        GameMove::Rock => if opp_move == GameMove::Paper {
            GameResult::Loss
        } else { GameResult::Win},
        GameMove::Paper => if opp_move == GameMove::Scissors {
            GameResult::Loss
        } else { GameResult::Win},
...
pub fn try_start_game(deps: DepsMut, info: MessageInfo, opponent: String, start_move: GameMove) -> Result<Response, ContractError> {
...
mod tests {
 ...
 //   #[test]
    fn start_game_validates() {
        ...
        let msg = ExecuteMsg::StartGame {opponent: oppo, start_move: GameMove::Scissors};
        let res = execute(deps.as_mut(), mock_env(), info, msg).unwrap_err();
        let error = ContractError::Std(
            StdError::GenericErr { 
        ...
        let info = mock_info("mario", &coins(1000, "earth"));
        let msg = ExecuteMsg::StartGame {opponent: oppo, start_move: GameMove::Rock};
        ...
        let oppo = String::from("badger");
        let msg = QueryMsg::GetGame {host: host, opponent: oppo};
  ...
  //  #[test]
    fn multi_game_test() {
        ...
        let info = mock_info("mario", &coins(1000, "earth"));
        let msg = ExecuteMsg::StartGame {opponent: oppo, start_move: GameMove::Paper};
  ...
  //  #[test]
    fn admin_test() {
        let mut deps = mock_dependencies(&[]);
        let msg = InstantiateMsg { admin: Some(INIT_ADMIN.into()) };
   ...
   // #[test]
    fn respond_to_game() {
        ...
        let msg = ExecuteMsg::Respond {host: host, opp_move: GameMove::Rock};
        execute(deps.as_mut(), mock_env(), info, msg).unwrap();
   ...
   // #[test]
    fn multi_games_leaderboard() {
        ...
        let host = String::from("mario");
        let info = mock_info("luigi", &coins(1000, "earth"));
        let msg = ExecuteMsg::Respond {host: host, opp_move: GameMove::Paper};
        execute(deps.as_mut(), mock_env(), info, msg).unwrap();
        ...
        let host = String::from("mario");
        let info = mock_info("luigi", &coins(1000, "earth"));
        let msg = ExecuteMsg::Respond {host: host, opp_move: GameMove::Scissors};
        execute(deps.as_mut(), mock_env(), info, msg).unwrap();
